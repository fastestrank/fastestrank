...
7. **How this report was made** — opens with the skill link line from `seo-report`, pointing at `https://www.fastestrank.com/docs/skills/local-seo` ("FastestRank Local SEO skill"), then which tools returned what, plus a note reading each missing grid rank against that point's `resultsCount` rather than calling it invisibility.
