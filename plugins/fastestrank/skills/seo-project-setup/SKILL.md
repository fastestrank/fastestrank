...
description: Populate a project's shared FastestRank context — site scope, goals, positioning, competitors, key pages, and preferences — plus MCP checks and Search Console intake.
---

# FastestRank SEO Project Setup

## Goal

Interview the user once about one website or SEO project, and store the answers in that project's shared context in FastestRank with `update_project_context`. That context is read by every other skill, by Danny in the app, and by the user on the project's Context page (in the sidebar under AI) — so it survives new sessions, new machines, and new agents. This is a context setup workflow, not a full audit.
...
### 1. Verify FastestRank MCP and resolve the project
...
6. If the MCP is unavailable, tell the user to connect FastestRank MCP; without it, nothing can be saved.

Do not run research tools just to test connectivity; `whoami` and `list_projects` are enough.

### 2. Read what is already there

Call `get_project_context`. Show the user a short summary of what FastestRank already knows and what is missing. Confirm or correct existing entries rather than re-asking questions that are already answered — this skill is often re-run after another skill filled in part of the context.
...
Project knowledge lives in FastestRank, not on disk. A local folder is still useful for the things that are actually files: GSC CSV exports, crawls, drafts, briefs, and reports.
...
After intake, recommend one next FastestRank workflow:
...
- FastestRank MCP/project status
- Sites in scope
- Goals
- Known positioning
- Competitors saved
