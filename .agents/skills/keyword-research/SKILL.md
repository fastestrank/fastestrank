...
# FastestRank Keyword Research

## Goal

Turn seed topics into a prioritized keyword opportunity set using FastestRank MCP data. The output should help the user decide what to target, what to save, and what to research next.
...
7. **How this report was made** — opens with the skill link line from `seo-report`, pointing at `https://www.fastestrank.com/docs/skills/keyword-research` ("FastestRank Keyword Research skill"), then which tools returned what.

## Guardrails

- Do not invent metrics. If FastestRank does not return a value, write `unknown`.
